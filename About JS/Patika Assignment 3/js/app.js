...
// To fill the site with all food menu whem page is opens.
document.addEventListener('DOMContentLoaded', writeItems(menu));
...
for (let item of categories) {
  ...
  newBtn.classList = `btn btn-outline-dark btn-item`;
  newBtn.innerHTML = item;
  btnMenu.appendChild(newBtn);
}

function writeItems(category) {
  sections.innerHTML = ``;
  for (let item of category) {
    let div = document.createElement(`div`);
    div.classList = `menu-items col-lg-6 col-sm-12`;
...
btnMenu.addEventListener(`click`, function (e) {
  ...
  if (btn.innerHTML === `All`) {
    newMenu = [...menu];
  } else if (categories.includes(btn.innerHTML)) {
    newMenu = menu.filter(item => item.category === btn.innerHTML);
  }
  writeItems(newMenu);
})
